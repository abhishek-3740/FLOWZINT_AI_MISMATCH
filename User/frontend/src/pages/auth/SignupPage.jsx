...
export default function SignupPage() {
  ...
  const [form, setForm] = useState({ name: 'Mira', email: 'mira@example.com', password: 'demo-demo' });
  ...
  const createDemoAccount = () => {
    signup({ name: 'Demo Shopper', email: 'demo@flowzint.com' });
    navigate('/', { replace: true });
  };
  ...
  return (
    <main className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl place-items-center px-4 py-12 sm:px-6 lg:px-8">
      <section ref={panelRef} className="grid w-full gap-10 overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/80 shadow-soft lg:grid-cols-[0.9fr_1.1fr]">
        <div className="relative overflow-hidden bg-gradient-to-br from-white/5 via-slate-950 to-sage-200/10 p-8 lg:p-10">
          <div className="absolute inset-0 grain-overlay opacity-40" />
          <div className="relative max-w-md">
            <p className="text-xs uppercase tracking-[0.34em] text-sage-200">Create account</p>
            <h1 className="mt-4 text-4xl text-white">Join the premium wellness storefront and unlock smart replenishment.</h1>
            <p className="mt-4 text-sm leading-7 text-slate-300">
              Signup follows the same animated form pattern so the onboarding stays consistent with the login experience.
            </p>
          </div>
        </div>
        ...
        <div className="p-8 lg:p-10">
          <form onSubmit={submitForm} className="space-y-5">
            ...
            <p className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-300">
              Demo account exists already: <span className="font-semibold text-white">demo@flowzint.com</span> / <span className="font-semibold text-white">demo1234</span>
            </p>

            <p className="text-center text-sm text-slate-400">
              Already have an account?{' '}
              <Link to="/auth/login" className="font-semibold text-sage-200 transition hover:text-white">
                Sign in
              </Link>
            </p>
          </form>
